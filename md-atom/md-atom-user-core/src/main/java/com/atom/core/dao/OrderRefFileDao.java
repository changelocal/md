...
public class OrderRefFileDao extends BaseDao {
    ...
    public void addBatch(List<OrderRefFile> list) {
        DB().insert(list).result();
    }

    public void updateBatch(List<OrderRefFile> list) {
        if (list.isEmpty()) {
            return;
        }
        list.forEach(p -> {
    ...
    public void delete(String orderNo) {
        DB().delete("order_ref_file").where(f("order_no", orderNo)).result();
    }
