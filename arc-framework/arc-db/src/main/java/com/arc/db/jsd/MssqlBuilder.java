...
/**
 * SQL Server 语句生成器
 * Created by xiuquan.tian on 15/5/25.
 */
        ...
        if (info.orders != null) {
            ...
            for (int i = 0; i < info.orders.size(); i++) {
                ...
                for (int j = 0; j < order.columns.length; j++) {
                    if (j > 0) buffer.addSql(",");
                    if (order.table != null) buffer.addSql("[%s].", order.table.getPrefix());
                    buffer.addSql("[%s]", order.columns[j]);
                }
                buffer.addSql(" %s", order.type.value);
            }
        }
